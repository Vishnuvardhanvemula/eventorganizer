...
export function StageBuilder({ onComplete, onBack, initialAddons = ["DJBooth"] }: Props) {
  ...
  return (
    <div className="fixed inset-0 w-full h-screen bg-[radial-gradient(ellipse_at_center,_var(--color-bg-primary)_0%,_var(--color-bg-secondary)_70%)] flex flex-col overflow-hidden">
      
      {/* Top Progress Bar */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-[var(--color-border)] z-50">
        <motion.div 
      ...
      >
        <button 
          onClick={onBack}
          className="flex items-center gap-2 font-sans text-xs tracking-[0.2em] uppercase text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          <span className="font-sans text-xs tracking-[0.2em] uppercase text-[var(--color-text-muted)] hidden md:block mt-1">
          ...
          <motion.button
            ...
            className="font-sans text-[10px] md:text-xs tracking-[0.2em] uppercase bg-[#C9A84C] text-[#FFFFFF] px-5 md:px-6 py-2.5 md:py-3 rounded shadow-[0_8px_24px_rgba(201,168,76,0.2)] hover:bg-[#b59540] transition-colors"
          >
            Get Proposal →
          </motion.button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          <motion.div 
            ...
            className="absolute bottom-32 md:bottom-12 left-1/2 -translate-x-1/2 bg-[var(--color-bg-secondary)]/80 backdrop-blur-md px-4 py-2 border border-[#C9A84C]/20 rounded-full shadow-sm pointer-events-none"
          >
            <span className="font-sans text-[10px] tracking-[0.2em] uppercase text-[var(--color-text-muted)] font-medium">
          ...
          {activeAddons.length === 0 && (
            <motion.p
              ...
              className="absolute bottom-24 left-1/2 -translate-x-1/2 font-sans text-xs tracking-[0.2em] uppercase text-[var(--color-text-muted)] text-center pointer-events-none"
            >
          ...
          <AnimatePresence>
            {upsellMessage && (
              <motion.div
                ...
                className="absolute bottom-40 md:bottom-24 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-50 backdrop-blur-xl bg-[var(--color-bg-secondary)] border border-[#C9A84C]/30 rounded-2xl p-5 shadow-[0_20px_60px_rgba(0,0,0,0.06)]"
              >
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <p className="font-sans text-sm text-[var(--color-text-primary)] leading-snug">{upsellMessage}</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={acceptUpsell}
                    className="flex-1 text-xs font-sans tracking-[0.15em] uppercase bg-[#C9A84C] text-[#FFFFFF] py-2.5 rounded hover:bg-[#b59540] transition-colors shadow-sm"
                  >
                    Add It
                  </button>
                  <button
                    onClick={dismissUpsell}
                    className="text-xs font-sans tracking-[0.15em] uppercase text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] py-2.5 px-4 transition-colors"
                  >
                    Skip
                  </button>
                </div>
        ...
        <button 
          ...
          className="md:hidden absolute bottom-6 left-1/2 -translate-x-1/2 bg-[var(--color-bg-secondary)] border border-[#C9A84C]/30 shadow-[0_8px_32px_rgba(201,168,76,0.15)] px-6 py-3 rounded-full flex items-center gap-2 z-40 transition-transform active:scale-95"
        >
          ...
          <span className="font-sans text-[11px] tracking-[0.2em] uppercase font-bold text-[var(--color-text-primary)]">
        ...
        <motion.aside
          ...
          transition={{ duration: 0.5, ease: [0.6, 0.05, -0.01, 0.9] }}
          className="absolute bottom-0 left-0 right-0 top-auto md:relative z-30 w-full md:w-80 xl:w-96 h-[60vh] md:h-full flex flex-col border-t md:border-t-0 md:border-l border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-[0_-20px_60px_rgba(0,0,0,0.1)] md:shadow-none overflow-hidden rounded-t-3xl md:rounded-none"
        >
          ...
          <div className="px-6 pt-6 pb-4 border-b border-[var(--color-border)] flex items-center justify-between">
            <div>
              ...
              <h2 className="font-serif text-2xl md:text-xl text-[var(--color-text-primary)] leading-none">Choose additions.</h2>
            </div>
            <button 
              className="md:hidden w-8 h-8 rounded-full bg-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)]"
          ...
          {/* Category Tabs */}
          <div className="flex items-center gap-0 border-b border-[var(--color-border)] px-4 overflow-x-auto hide-scrollbar">
            {categories.map((cat) => (
              <button
                key={cat}
                ...
                className={`px-3 py-4 font-sans text-[10px] md:text-[11px] tracking-[0.15em] uppercase transition-colors relative whitespace-nowrap ${
                  activeCategory === cat ? "text-[var(--color-text-primary)] font-medium" : "text-[var(--color-text-muted)] hover:text-[var(--color-text-muted)]"
                }`}
              >
                {cat}
                {activeCategory === cat && (
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full text-left p-4 rounded-xl border transition-all duration-300 relative overflow-hidden group ${
                        isActive
                          ? "border-[#C9A84C]/50 bg-[var(--color-bg-primary)] shadow-sm"
                          : "border-[var(--color-border)] bg-[var(--color-bg-secondary)] hover:border-[var(--color-border)] shadow-none"
                      }`}
                    >
                      {isActive && !isCore && (
                      ...
                      <div className="flex items-start gap-3 relative z-10">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                          isActive ? "bg-[#C9A84C] text-[#FFFFFF]" : "bg-[var(--color-bg-primary)] text-[var(--color-text-muted)]"
                        }`}>
                          <Icon size={18} strokeWidth={isActive ? 2 : 1.5} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            <span className={`font-sans text-sm font-medium ${isActive ? "text-[#C9A84C]" : "text-[var(--color-text-primary)]"}`}>
                              {module.label}
                            </span>
                            {isCore ? (
                              <span className="font-sans text-[9px] tracking-[0.15em] uppercase text-[var(--color-text-muted)] bg-[var(--color-border)] px-2 py-0.5 rounded flex items-center gap-1">
                              ...
                              <div className={`w-4 h-4 rounded-full border-[1.5px] transition-colors flex items-center justify-center ${
                                ...
                                <AnimatePresence>
                                  {isActive && (
                                    ...
                                    >
                                      <LucideIcons.Check size={10} className="text-[#FFFFFF]" strokeWidth={3} />
                                    </motion.div>
                                  )}
                                </AnimatePresence>
                              </div>
                          ...
                          <p className="font-sans text-[11px] text-[var(--color-text-muted)] leading-snug">{module.description}</p>
                          <p className={`font-sans text-[11px] mt-2 font-medium tracking-wide ${isActive ? "text-[#C9A84C]" : "text-[var(--color-text-muted)]"}`}>
          ...
          <div className="px-4 py-4 border-t border-[var(--color-border)] hidden md:block">
            <button
              onClick={() => onComplete(activeAddons)}
              className="w-full py-4 bg-[var(--color-text-primary)] text-[var(--color-bg-primary)] font-sans text-[10px] md:text-xs tracking-[0.25em] uppercase rounded flex justify-between items-center px-6 hover:bg-[var(--color-text-primary)]/90 transition-colors"
            >
