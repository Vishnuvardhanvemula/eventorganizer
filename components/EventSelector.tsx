    ...
    >
      {Object.values(events).map((event, index) => (
        <motion.button
          ...
          className="group relative h-80 flex flex-col items-center justify-center p-8 bg-neutral-50 hover:bg-neutral-100 border border-neutral-200 transition-colors cursor-pointer overflow-hidden rounded-sm"
        >
          <motion.div layoutId={`scene-content-${event.id}`} className="flex flex-col items-center">
            <h2 className="font-serif text-3xl text-neutral-800 group-hover:text-[var(--color-gold)] transition-colors z-10 text-center">
              {event.label}
            </h2>
            <p className="mt-4 text-sm text-neutral-400 font-sans tracking-[0.2em] uppercase opacity-0 group-hover:opacity-100 transition-opacity z-10">
              Select
            </p>
          </motion.div>
